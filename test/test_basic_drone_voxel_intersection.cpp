...
TEST(PointCloudVoxels, TestBarelyCollision) {
    ...
    RobotFactory::SharedPtr factory = std::make_shared<RobotFactory>(VoxelType::POINT_CLOUD);
    Roadmap::SharedPtr roadmap = factory->makeRoadmap("Test Drone Roadmap", RobotType::DRONE);
    auto v1_left = roadmap->addVertex("v1_left", std::make_shared<DroneState>(Point(0, 0, 10), ROBOT_RADIUS));
    auto v1_right = roadmap->addVertex("v1_right", std::make_shared<DroneState>(Point(0, 0, 1), ROBOT_RADIUS));
    // essentially 2.1
    auto v2_left =
        roadmap->addVertex("v2_left", std::make_shared<DroneState>(Point(0, 0, VOXEL_RESOLUTION), ROBOT_RADIUS));
...
TEST(DronePointCloudVoxels, TestParallelNearbySameRoadmap) {
    //
